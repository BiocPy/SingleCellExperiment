...
def test_combine_rows(experiments):
    combined = biocutils.combine_rows(experiments.se_unnamed, experiments.se_unnamed_2)
...
def test_relaxed_combine_rows(experiments):
    ...
    with pytest.raises(Exception):
        combined = biocutils.combine_rows(experiments.se_unnamed, test2)

    combined = biocutils.relaxed_combine_rows(experiments.se_unnamed, test2)
    assert combined is not None
    assert isinstance(combined, SingleCellExperiment)
    assert len(combined.alternative_experiments) == 0
    assert len(combined.row_data["A"]) == 200
